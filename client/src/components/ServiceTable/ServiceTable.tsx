...
import AdminFormUpdateServices from '../AdminFormUpdateServices/AdminFormUpdateServices'
import { useSelector } from 'react-redux'
import { RootState } from '../../redux/store'

export const ServiceTable: React.FC<{ hasError: boolean }> = ({ hasError }) => {
	...
	const getProducts = () => {
		try {
			customAxios.get(`${process.env.REACT_APP_SERVER_URL}/services/all`).then(({ data }) => {
				setFetchData(data)
	...
	const deleteProducts = () => {
		try {
			if (window.confirm(`Вы действительно хотите удалить все услуги?`)) {
				customAxios.delete(`${process.env.REACT_APP_SERVER_URL}/services/all`).then(() => {
					getProducts()
				})
			}
		} catch (error) {
	...
	const deleteItem = (id: string, title: string | undefined) => {
		try {
			if (window.confirm(`Вы действительно хотите удалить услугу ${title}?`)) {
				customAxios.delete(`${process.env.REACT_APP_SERVER_URL}/services/id/${id}`).then(() => {
					getProducts()
				})
			}
		} catch (error) {
		...
		<div className='admin__list-main-block'>
			<div className='admin__list-header'>
				<div className='admin__list-header-block'>
					<h3 className='admin__list-title'>Услуги</h3>
