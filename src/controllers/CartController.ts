		...
		jwt.verify(token, `${process.env.JWT_SECRET}`, (err: jwt.VerifyErrors | null, decoded: any) => {
			if (err) {
				...
				if (req.body.act === 'push') {
					pool.query(
						'INSERT INTO carts (id, user_id, product_id, totalcount, res_price) VALUES ($1, $2, $3, 1, $4)',
						[uuidv4(), decoded.id, req.body.product, req.body.price],
				...
				if (req.body.act === 'plus') {
					pool.query(
						...
						(error: Error, results: QueryResult) => {
							if (error) throw error
							pool.query(
								'SELECT c.*, p.title, p.price, p.count, p.discount, p.imgurl FROM carts c JOIN product p ON c.product_id = p.id WHERE c.user_id = $1',
								[decoded.id],
								(error: Error, results: QueryResult) => {
				...
				if (req.body.act === 'minus') {
					pool.query(
						...
						(error: Error, results: QueryResult) => {
							if (error) throw error
							pool.query(
								'SELECT c.*, p.title, p.price, p.count, p.imgurl FROM carts c JOIN product p ON c.product_id = p.id WHERE c.user_id = $1',
								[decoded.id],
								(error: Error, results: QueryResult) => {
									return res.status(200).json({ results: results.rows })
								},
