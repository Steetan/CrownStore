...
export const getProductsPerip = (req: Request, res: Response) => {
	pool.query(
		'SELECT * FROM product WHERE category >= 8 AND category <= 10',
...
export const updateProduct = (req: Request, res: Response) => {
	pool.query(
		'UPDATE product SET title = $1, description = $2, price = $3, category = $4, imgurl = $5, count = $6, discount = $7 WHERE id = $8',
		[
			req.body.title,
		...
		(error: Error, results: QueryResult) => {
			if (error) throw error
			pool.query(
				'UPDATE carts SET totalcount = p.count FROM product p WHERE carts.product_id = p.id AND (carts.totalcount > p.count OR (carts.totalcount = 0 AND p.count <> 0))',
				(error: Error, results: QueryResult) => {
					if (error) throw error
...
export const createProduct = (req: Request, res: Response) => {
	try {
		...
		pool.query(
			'INSERT INTO product (id, title, description, price, category, imgurl, rating, count, discount) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)',
			[
				uuidv4(),
				req.body.title,
				req.body.description,
	...
	try {
		pool.query('DELETE FROM product', (error: Error, results: QueryResult) => {
			...
			pool.query('DELETE FROM carts', (error: Error, results: QueryResult) => {
				if (error) throw error
				pool.query('DELETE FROM reviews', (error: Error, results: QueryResult) => {
					if (error) throw error
