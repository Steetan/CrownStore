...
export const postEmail = (req, res) => {
    ...
    jwt.verify(token, `${process.env.JWT_SECRET}`, (err, decoded) => {
        if (err) {
            ...
            pool.query(`SELECT carts.*, users.name_user AS user_name, users.fname_user AS user_fname, users.oname_user AS user_oname, users.email AS user_email, users.phone_number AS user_phone, product.price
				FROM carts INNER JOIN users ON carts.user_id = users.id INNER JOIN product ON carts.product_id = product.id WHERE carts.user_id = $1`, [decoded.id], (error, results) => {
                if (error)
                    throw error;
                pool.query('SELECT title, price FROM services WHERE id = ANY($1)', [req.body.services], (error, resultsServices) => {
                    if (error)
                        throw error;
...
export const postEmailComplaint = (req, res) => {
    pool.query(`SELECT reviews.*, users.name_user AS user_name, users.fname_user AS user_fname, users.oname_user AS user_oname, product.title
		FROM reviews 
		INNER JOIN users ON reviews.user_id = users.id 
		INNER JOIN product ON reviews.product_id = product.id 
		WHERE reviews.id = $1`, [req.params.id], (error, results) => {
        if (error)
            throw error;
        const transporter = nodemailer.createTransport({
            service: 'Gmail',
...
export const postEmailIdea = (req, res) => {
    const token = (req.headers.authorization || '').replace(/Bearer\s?/, '');
    jwt.verify(token, `${process.env.JWT_SECRET}`, (err, decoded) => {
        if (err) {
            res.status(400).json({ error: 'Неверный токен' });
            ...
            pool.query(`SELECT name_user, fname_user, oname_user, email FROM users WHERE id = $1`, [decoded.id], (error, results) => {
                if (error)
                    throw error;
                const transporter = nodemailer.createTransport({
                    service: 'Gmail',
